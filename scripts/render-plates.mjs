...
</style></head><body><div class="plate-frame">${svg}</div></body></html>`;
}

function cropHtml(svg, theme, plateW, plateH, x0, y0, x1, y1, zoom) {
  const css = readFileSync(GLOBAL_CSS, 'utf8');
...
</style></head><body><div class="crop-frame"><div class="crop-inner">${svg}</div></div></body></html>`;
  return { html, cw, ch };
}
...
async function main() {
  ...
  for (const sheet of args.sheets) {
    ...
    for (const theme of args.themes) {
      const html = pageHtml(result.svg, theme, plateW, plateH);
