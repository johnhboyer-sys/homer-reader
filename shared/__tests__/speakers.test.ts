...
describe('lineRenderParts — editorial sigla inside a word', () => {
  // No line in this corpus prints a siglum inside a word today; the sister
  // repos do (OCT Plato "ἔπει<τα>", "[προς]θῶμεν"). There the bare token
  // surface doesn't occur verbatim, a plain indexOf missed it, and the old
  // fallback printed the word twice. A supplemented word must print ONCE, in
  // its bracketed form, and stay clickable.
  ...
  it('leaves a phrase-level closer outside a word whose bracket closed mid-word', () => {
    // OCT Cratylus 389e "ἀ<μφι>γνοεῖν" and Laws 756a "ἀντι<προ>βολὴν" close their
