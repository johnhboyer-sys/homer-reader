...
// Editorial sigla an edition can print INSIDE a word: angle brackets for a
// supplement, square brackets for a deletion (Allen prints both; sister repos
// hit this on OCT Plato, e.g. "ἔπει<τα>", "[προς]θῶμεν"). The token text is the
// bare word, so it does not occur verbatim in the line and a plain indexOf
// misses it. No line in this corpus does this today — the guard is here so a
// future text cannot make a word print twice.
const SIGLUM = /[<>[\]]/;
const OPENER = /[<[]/;
const CLOSER = /[>\]]/;
