...
export interface SegMeta {
  ...
  column: string;
  greek_head: string;
  // Space-joined fold token sequence, emitted by stage6_search.py. Kept in the
  // schema (and in meta.json) because other consumers read it, but search no
  // longer matches phrases against it: it is built skipping keyless tokens
  // while the postings count every token, and it keeps only lemmata[0]. Greek
  // phrases go through phraseStarts() over the postings instead.
  greek_tokens: string;
  english_head: string;
}
...
export interface Offsets {
  ...
  // Homer has no chapter division, so the pipeline emits this empty and every
  // guard on chapStarts.length below no-ops. The field is kept so the client
  // stays one shape across the three readers.
...
// whole tokens (and a wildcard hits whatever its pattern licenses). 'phrase'
// returns each phrase's start offset; 'all'/'any' return every matching token.
// One offset = one rendered occurrence, so repeats past the old 500-char cap
// now count and show — which is why this and not a boolean phrase test: the
// English KWIC and the per-segment instance count are both built from these
// offsets.
function engMatchTerm(word: string, term: string): boolean {
  const pattern = compilePattern(term, englishFold);
  if (!pattern) return false;
  return 'exact' in pattern ? word === pattern.exact : pattern.test(word);
}
...
export interface SearchResult {
  ...
  grkMatch: boolean;
  engMatch: boolean;
  // Token positions in the segment where a Greek term matched. These index the
  // FLATTENED surface token list of the segment's Greek (every token counted,
  // keyless ones included) — the same space Search.svelte's accent post-filter
  // and greekKwic walk. Any future "compress out keyless tokens" optimisation
  // in the postings would silently corrupt both.
  grkPositions: number[];
...
//
// `line` is the EMITTED verse number, not an index: line_runs stores the number
// the vulgate gives the line, so the walk below does no index arithmetic and a
// numbering gap (Od. 10.455 -> 10.457, 16.100 -> 16.102, 23.48 -> 23.50) simply
// makes two adjacent runs non-consecutive. A resolver that counted lines
// instead would cite 456 for a line the tradition calls 457.
//
// A segment IS a book here (24 per work, seg.column === String(seg.book)), so
// `column` and `book` always agree; the caller composes the citation through
...
export function offsetRef(offsets: Offsets, global: number): OffsetRef | null {
  ...
  for (const [line, count] of seg.line_runs) {
    // A zero-token run can never own an offset (left < 0 is false), so it is
    // skipped rather than mis-claiming the next line's first token.
    if (left < count) return { seg_idx, pos, book: seg.book, column: seg.column, line };
    left -= count;
  }
  // Sum of the run counts is short of the segment's token span: a build defect.
  // Return null rather than guess a line number.
  return null;
}
...
// 'line' is a VERSE here, and it is the interesting unit for Homer: a formula
// is a within-verse phenomenon. 'chapter' is inert — Homer emits no chapter
// bounds — and the UI does not offer it; the value is kept so the three readers
// share one type.
...
// The proximity default: 5 words, which is most of a hexameter line (the
// corpus averages 7.1 tokens per verse in the Iliad, 7.2 in the Odyssey).
// Capped at 50 — about seven verses — since past that "near" stops meaning
// anything.
  ...
  for (const w of windows) {
    ...
    for (const h of w.hits) {
      for (let k = 0; k < h.span; k++) {
        ...
        // Mark the token in whichever segment it actually falls in. In Homer
        // a segment IS a book, so a window — which may never cross a book edge
        // — always lands in one segment and this loop never splits a hit. The
        // per-token locate() is kept anyway: it is what makes the code correct
        // in the sibling readers, where a book holds several columns.
...
export async function searchCombo(
  ...
  // Only worth saying when the answer depends on where a chapter begins — so
  // never in Homer, whose chapter_bounds is empty. Kept unpopulated rather than
  // deleted, to hold divergence from the sibling readers down.
