<script lang="ts">
  ...
  // One resolved occurrence. `line` is null for English: the translation is
  // aligned a block per segment and a segment is a book, so the citation names
  // the book and there is no verse to give.
  interface Occurrence {
    ...
    ref: string;
    href: string;
    // Does the whole phrase stand inside one verse? Always true for English,
  ...
  interface WorkCitations {
    id: string;
    title: string;
    total: number;               // occurrences in this work, before any filtering
  ...
  // same phrases rather than each inventing its own. Counts are the built
  // corpus's own, not remembered figures.
  const EXAMPLES = [
    { beta: 'epea pteroenta proshuda', greek: 'ἔπεα πτερόεντα προσηύδα', gloss: 'spoke winged words — 107 times' },
    { beta: "ton d' apameibomenos prosefh", greek: "τὸν δ' ἀπαμειβόμενος προσέφη", gloss: 'then in answer spoke — 55 times' },
    { beta: "ws efaq' oi d' ara", greek: "ὣς ἔφαθ', οἱ δ' ἄρα", gloss: 'so he spoke, and they — 54 times' },
  ];
  // Chosen the same way, and each standing in both poems so it is Homer
  // recurring rather than one translator's habit.
  const ENGLISH_EXAMPLES = [
    { beta: 'odysseus of many wiles', greek: 'odysseus of many wiles', gloss: '92 times, both poems' },
    { beta: 'the son of atreus', greek: 'the son of atreus', gloss: '78 times, both poems' },
    { beta: 'the goddess flashing eyed', greek: 'the goddess flashing eyed', gloss: '54 times, both poems' },
  ];
  // The Greek streams are sharded on fold letters, which have no j and no v;
  // the English stream is sharded on the English alphabet.
  const GREEK_LETTERS = 'abcdefghiklmnopqrstuwxyz'.split('');
  ...
  // small words in fixed order — "of the" is nearly the only order those two
  // ever take — so the measure reads them as inseparable and they take the whole
  // top of the list: of the (2,736), to the (1,277), in the (1,194). Greek does
  // that work with endings instead, so its small words move freely and do not
  // clump as hard.
  //
  ...
  // carry weight in Homer — no ship, spear, wine, heart, host, hand.
  //
  // Known cost, inherited from the sibling reader's rule and kept for
  // consistency: a phrase needs two content words, so real formula-elements
  // built on one go too — of the Achaeans (318), the Achaeans (536), so spake
  // (259). The named epithet-formulae that make the page worth reading survive:
  // Odysseus of many wiles, the son of Atreus, the goddess flashing eyed.
  ...
  let hideCommon = true;
  // Homeric formula is largely a within-verse phenomenon, so the interesting
  // occurrences are the ones that stand inside a single verse. On by default for
  // the Greek streams; the question does not arise for English, whose offsets
  ...
  $: browseLetters = isEnglish ? ENGLISH_LETTERS : GREEK_LETTERS;
  // The toggle is a Greek-stream control: an English occurrence resolves to the
  // book, so "inside one verse" has nothing to test.
  $: verseFilterOn = withinVerse && !isEnglish;
  ...
  // Nothing is shown until something is typed, so nothing is fetched either:
  // the sibling warms a default shard on load, which here would be a third of a
  // megabyte for a screen that shows the examples panel.
  $: if (mounted && normalizedPrefix && shardSignature && shardSignature !== requestedShardSignature) {
  ...
  // one ambiguous word asks for three shards and two are routinely dead ends.
  ...
  // the ones that were tried — an ambiguous word licenses several headwords and
  // most are dead ends. Listed rather than counted, because a reader has to be
  // able to see which dictionary word their word was read as; long lists are cut
  // off rather than allowed to bury the page.
   ...
   *
   * The dictionary-form index is keyed on headwords, so a phrase a reader has in
   * front of them matches nothing typed literally: τὸν δ' ἀπαμειβόμενος is
   * stored under ὁ δέ ἀπαμείβομαι, and knowing that is exactly the knowledge the
   * index is supposed to save them. So resolve each typed word to the headwords
   * it can belong to and match every reading of the phrase.
   *
   ...
   * is always among its own headwords, so adding it back changes no result and
   * can cost a whole extra shard for nothing.
   */
  async function makePlan(
    ...
    // Stamped synchronously, before the first await: a consumer checks
    // `plan.key === planKey` and renders nothing for a query already replaced.
  ...
  async function loadPhraseDetails(id: string, item: PhraseItem) {
    ...
    // The shard the ROW lives in, which after widening is not the typed letter.
    const rowLetter = shardLetter(item.key);
    const span = item.row[0];
    const forEnglish = isEnglish;
    try {
      const occurrenceShard = await fetchNgramOccurrences(stream, rowLetter, span);
      ...
      const englishSegments = forEnglish ? await fetchEnglishSegments() : null;

      await pool(entries, 4, async ([work, deltas], index) => {
        try {
          // The English is aligned a block per segment and a segment is a book,
          // so an English offset resolves to the book and stops there. A Greek
          // offset resolves to the verse, because the Greek is indexed per token.
          let occurrences: Occurrence[];
          if (englishSegments) {
            const segs = englishSegments[work] ?? [];
            occurrences = decodeOffsets(deltas)
              .map((global) => {
              ...
              .map((seg) => ({
                ...
                line: null,
                book: seg.book,
                ref: formatCite(work, seg.column, null),
                href: `${BASE_URL}${workPath(work, seg.book)}?loc=${formatLocValue(work, seg.column, null)}`,
                sameVerse: true,
              }));
          } else {
            const offsets = await fetchOffsets(work);
            const starts = lineStartsOf(offsets);
            occurrences = decodeOffsets(deltas)
              .map((global): Occurrence | null => {
                ...
                return {
                  ...
                  line: ref.line,
                  book: ref.book,
                  ref: formatCite(work, ref.column, ref.line),
          ...
          groups[index] = { ...groups[index], occurrences };
        } catch {
          groups[index] = {
            ...groups[index],
  ...
  // Applied at render, not at fetch, so flipping the toggle re-counts what is
  // already resolved instead of asking for the offsets again.
  function shownOccurrences(group: WorkCitations, verseOnly: boolean): Occurrence[] {
    return verseOnly ? group.occurrences.filter((o) => o.sameVerse) : group.occurrences;
  }
</script>
...
<main class="phrases-page">
  <header class="page-intro">
    ...
    <h1>Phrases</h1>
    <p>
      Every two- to five-word sequence Homer uses more than once: 68,550
      surface-form phrases, 158,973 dictionary-form phrases and 118,695 phrases
      of the translations, counted across both poems.
  ...
  <section class="phrase-panel" aria-labelledby="phrase-filters">
    ...
    {#if !isEnglish}
      <label class="field verse-toggle">
        <span>
          <input type="checkbox" bind:checked={withinVerse} />
          Count only occurrences inside a single verse
        </span>
        <small>
          Homeric formula is largely a within-verse thing, so this is the view
          worth having first. The index keeps every occurrence either way: clear
          this to see the ones that run over a verse end.
        </small>
      </label>
    {/if}
    ...
    <label class="field prefix-field" for="phrase-prefix">
      ...
      <small>
        {#if isEnglish}
          Type the English words as they stand in the translation. An English
          phrase is located to the book, not to the verse: the translations are
          aligned a block to a book, so there is no line to name.
        {:else if isLemma}
          Type Greek or plain letters, as the words stand in the verse: this list
          stores <span lang="grc">τὸν δ' ἀπαμειβόμενος</span> as
          <span lang="grc">ὁ δέ ἀπαμείβομαι</span>, and either finds it.
        {:else}
          Type Greek or plain letters — <span lang="grc">ἔπεα πτερόεντα</span>
          and <code>epea pteroenta</code> both work. Accents are ignored.
        {/if}
      ...
      {#if isEnglish}
        <label class="field common-words">
          ...
          <small>
            ...
            <em>of the</em> and <em>in the</em> outrank anything Homer says. This
            hides them; it changes no count and no score.
          </small>
        </label>
      {/if}
    ...
    <div class="work-field">
      <!-- Checkboxes, not a multi-select list. A list box needs a modifier key
           nobody is told about, and one stray click throws the whole selection
           away — the same control the sibling readers use, so the three pages
           behave alike. -->
      ...
      <div class="work-meta">
        <p id="work-filter-note">
          No selection includes both poems. Tick one or more to keep phrases
        ...
        {#if selectedWorks.length}
          <button type="button" class="quiet-button" on:click={clearWorks}>
            Clear poem filter ({selectedWorks.length})
          </button>
        {/if}
      </div>
    </div>
  ...
  <section class="results" aria-labelledby="phrase-results">
    <div class="results-head">
      ...
      <div class="results-tools">
        <!-- Sort lives here, above the list, not among the filters: it orders
             results, it does not remove any, and buried in a filter panel nobody
             sees which order they are looking at. -->
    ...
    {#if !normalizedPrefix}
      <div class="phrase-start">
        <p>
          Every run of two to five words that Homer uses more than once, counted.
          Start with one of these, or type a phrase above.
          {#if isEnglish}
            These are the translations, so a phrase standing in one poem alone
            may be the translator's habit; one standing in both is Homer
    ...
    {:else if workFilterLoading}
      <p class="status" aria-live="polite">Loading occurrence data for the poem filter…</p>
              ...
              >
                ...
                {:else if itemDetails}
                  {#each itemDetails.works as group}
                    {@const shown = shownOccurrences(group, verseFilterOn)}
                    <section class="work-citations" aria-labelledby={`phrase-${id}-${group.id}`}>
                      <div class="work-heading">
                        <h3 id={`phrase-${id}-${group.id}`}>{group.title}</h3>
                        <span>{countFormat.format(shown.length)}</span>
                      </div>
                      {#if group.error}
                        <p class="detail-status error">{group.error}</p>
                      {:else}
                        <ul class="citation-list">
                          {#each shown.slice(0, CITATION_CAP) as citation}
                            <li>
                              <a href={citation.href}>{citation.ref}</a>
                            </li>
                          {/each}
                        </ul>
                        {#if verseFilterOn && shown.length < group.total}
                          <p class="cap-note">
                            {countFormat.format(group.total - shown.length)} more
                            {group.total - shown.length === 1 ? 'occurrence runs' : 'occurrences run'}
                            over a verse end and {group.total - shown.length === 1 ? 'is' : 'are'} not counted here.
                          </p>
                        {/if}
                        {#if shown.length > CITATION_CAP}
                          <p class="cap-note">
                            Showing {CITATION_CAP} of {countFormat.format(shown.length)}
                            occurrences.
                          </p>
                        {/if}
                      {/if}
