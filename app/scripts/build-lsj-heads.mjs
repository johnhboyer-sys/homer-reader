...
// shard per lookup: 4.7 MB of public/data/lsj/e.json to learn how to spell a
// word, with every sense and citation in it thrown away unread. Homer's shards
// total 34 MB.
//
// public/data/lemmata.json cannot serve this: it is the lemma-PAGE manifest,
// scoped to lemmata that have a page, so it holds only a fraction of the keys
// the analyses name — and the ones it misses are the commonest words in the
// corpus (ὁ, καί, δέ, γάρ, μέν), which are exactly the words a reader taps.
//
// Ported from aristotle-reader app/scripts/build-lsj-heads.mjs. Homer-specific:
// analyses here also carry `cunliffe` keys, which this manifest does NOT cover —
// the Cunliffe presentation is a separate, later question (John, 2026-08-30), so
// Cunliffe still reads its own shards.
//
...
// holds: an entry no analysis points at can never surface in a card. Homer's
// data dir also holds non-work directories (cunliffe, lemmata, plates, …);
// requiring analyses.json is what separates a work from the rest.
...
// that letter; never derive one from the key's trailing digit. The digit and
// the letter disagree (ka/r2 is LSJ's (A)), and most numbered keys carry no
// letter at all — those get none rather than an invention.
const HOM = /^\s*\S+\s*\(([A-Z])\)/;
...
for (const f of readdirSync(shardDir)) {
  ...
  for (const [key, entry] of Object.entries(shard)) {
    ...
    if (m) rec.hom = m[1];
    // LSJ's own one-line sense for THIS entry (91% of entries carry one).
    // Homer-specific, and the reason this manifest is 156 KB gzipped rather
    // than 82: the popup gives each dictionary-level homonym its own
    // definition — ἔχω (A) "have, hold" beside ἔχω (B) "bear, carry" — and
    // that came out of the shard before. Dropping it to save the bytes would
    // regress a feature this reader already has.
    if (entry.short && String(entry.short).trim()) rec.short = String(entry.short).trim();
    out[key] = rec;
  }
}
...
// a silent gap ships: anything checking only the exit status would call an
// incomplete manifest a success. A key with no shard entry means the popup
// falls back to transliterating the lemma for that word, which looks like a
// headword bug somewhere else entirely.
if (works === 0) {
