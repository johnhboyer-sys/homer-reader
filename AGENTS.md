...
`~/Developer/aristotle-reader`): consult their AGENTS.md/docs for shared-machinery
...
- Git flow (John, 2026-07-17; two-remote split 2026-07-18 post-launch):
  ...
  (John): PR #1 = Codex/build → main umbrella (phases 0–3 + scenes);
  subsequent coherent units get branches off Codex/build with stacked PRs
  into Codex/build. Review gate applies at PR time; merging `main` is
...
- Codex gotcha (machine-level, inherited): after a ChatGPT account switch, the
  ...
  (`~/.Codex/plugins/data/codex-openai-codex/state/<repo-hash>/`) holds threads
...
- **Usage rebalance (John, 2026-07-17 ~17:00): conserve Codex; lean on
  Codex + Grok.** Codex 7d at 57%, Codex has two resets banked, Grok at
  ...
  **Escalation rule (John, same day, refined): minor stumbles stay in
  ...
  to Codex (Sonnet; Opus on difficulty) only on a BAD fuckup: badly
  wrong output, a failed verification gate traceable to the agent's
  sloppiness, or a second failure on the same brief. Codex is the
  trust anchor; economy never outranks correctness.**
  **Cross-model brief discipline (John, same day): Codex/Grok briefs
  must be tighter than Codex briefs — zero implicit context. Spell
  ...
  **Codex output verification (John, same day: "it can be sloppy"):
  every Codex implementation lane gets a quick Codex (Sonnet)
...
matching the label** (caught by John 2026-07-16). CLI forwarders run a Codex
...
family* (Sol reviewing Codex-written code, or Opus reviewing Codex-written
...
The four core rules, from Karpathy's AGENTS.md:
