...
> **(b) is contested.** Hainsworth on 12.118–19 reads the same words from the **attacker's**
> standpoint and equates them with the camp's own **right**. The dossier had (b) from a review
> of Clay. It now has a first read commentary authority on the other side. Recorded, not
> resolved — and see the flagged verdict at the end of this section, which gates drawing work.
...
- **claim (b), sense — DISPUTED as of 2026-07-30** — that the axis is the left and right of an
  ...
  (via Marks) says yes; Hainsworth on 12.118–19 says the poem's `ἀριστερά` at that line is
  Asios's left, hence the camp's right.
...
> 3. **Janko vol. IV on 13.675 and 13.681–84** — the standard modern treatment of the ships'
>    left, and of the sector Hector breaks into. Confirmed **absent from archive.org lending**
>    as of 2026-07-30: ILL only (§8 item 4).
...
close C-9; a commentator will, or nothing will.
...
- **The trilemma, stated so nobody resolves it by fiat.** Either (i) 12.118's ships'-left and
...
4. **Richard Janko, vol. IV, Books 13–16** (Cambridge, 1992) — **confirmed ABSENT from
   archive.org controlled-digital lending as of 2026-07-30**, after the borrow session that
   obtained items 3, 5 and 6 from the same series. Not a search failure this time: the other five
   volumes have lending records and this one does not. **ILL or a library visit only.** Wanted:
   notes on **13.681–84** and **16.285–86** (the Protesilaus contradiction, C-3 — Janko is the
   standard modern treatment), **13.675** and the ships'-left sector (**now also C-9**, where
   Janko is the obvious modern authority on Hector's ignorance and on where the breach was), and
   ...
   *Would settle:* C-3, part of C-9, and the map question. **This is now the highest-value ILL
   request the project has.**
