...
The handoff you already have (`docs/advanced-search-handoff.md`) describes a
phrase index whose typed prefix names one shard. **That design is wrong, and
this addendum replaces it.** Build it this way the first time; retrofitting cost
a 325-line diff there.
...
the nine tests in aristotle's `shared/__tests__/phrases.test.ts` do.
...
## Homer notes (added by the orchestrator, 2026-07-28)

- Homer's corpus is **two works**, not 41, so §5's checkbox-list rationale is
  weaker here — but copy the control anyway; consistency across the three
  readers is worth more than saving one widget.
- The within-one-verse toggle (main handoff §3) is Homer-only and lives beside
  the stream radio. It is a query-time filter over `line_runs`; the build must
  never pre-filter straddling phrases.
- `lemma-map/` **is** emitted by Homer's `stage8_ngrams.py` — the port did not
  skip it.
